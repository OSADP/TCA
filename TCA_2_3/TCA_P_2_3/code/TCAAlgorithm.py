...
class TCA_Algorithm:
    def __init__(self, CF, RSEs, RSE_tree, RandomGen_seed, Regions, SPOT_tree, msg_limit = 800000):
        ...
        self.random_generator.add_generator_int('SPOT_Station_ID', 1, 4294967295) #based SPOT standard
        ...
        self.Travel_SPOT_col_order = ['localtime', 'vehicle_ID', 'x', 'y', 'spd']
        self.Behavior_SPOT_col_order = self.Travel_SPOT_col_order + ['accel', 'heading', 'yawrate']
    ...
    def pull_veh_data(self, veh, tp):
        ...
        for v in veh:
            vehicle_data.append(self.tbl.pull_veh_data(veh = v, CF = self.CF, RandomGen = self.random_generator, Regions = self.regions, tp = tp))
    ...
    #-------------------------------------------------------------------------
    def BSM_tmp_ID_check(self, veh_data, tp):
        """
        ...
        if (veh['total_dist_traveled'] >= self.CF.Strategy['DistanceBeforePDMCollection']) or \
           (veh['total_time_in_network'] >= self.CF.Strategy['TimeBeforePDMCollection']):
                ...
                if (veh['time_motionless'] >= self.CF.Strategy['StopThreshold']) and \
                    (((tp - veh['time_of_last_stop'] >= self.CF.Strategy['StopLag']) and \
                    (veh['looking_for_start'] == True)) or (veh['looking_for_start'] == False)):
...
#-------------------------------------------------------------------------
    def GenerateTransmitCAM(self, veh_data, transTo, tp):
        ...
        if veh_data['CAM_equipped']:
            ...
            CAM = {
                ...
                'protocolVersion' : 1,
                'messageID' : 2,
                'stationID' : veh_data['SPOT_Station_ID'],
    ...
    def Write_PDMs(self, CF, clear_buffer=False):

        if (len(self.PDMs) >= self.msg_limit) or clear_buffer:
            df = pd.DataFrame(self.PDMs)
            ...
            # df['Message_SS_Number'] = df['Message_SS_Number'].fillna(99999)
            # df['Vehicle_SS_Number'] = df['Vehicle_SS_Number'].fillna(99999)

            df.to_csv(path_or_buf =CF.Control["PDMAllFile"],
                           index=False,
                           mode='a',
            ...
            df.to_csv(path_or_buf =CF.Control["PDMTransFile"],
                           index=False,
                           mode='a',
                           header = self.headerPDM,
                           cols=['DSRC_MessageID',
    ...
    def Write_BSMs(self, CF, clear_buffer=False):

        if (len(self.BSMs) >= self.msg_limit) or clear_buffer:
            logger.info('WRITE_BSMS')
            df = pd.DataFrame(self.BSMs)
            df = df.sort(['transtime', 'Vehicle_ID'])

            df['avg_accel'] = df['avg_accel'].fillna(-9999.0)
            df['heading'] = df['heading'].fillna(-9999.0)
            df['avg_accel'] = df['avg_accel'].map(lambda x: '%.3f' % x)
            df['spd'] = df['spd'].map(lambda x: '%.3f' % x)
            ...
            df.to_csv(path_or_buf =CF.Control["BSMTransFile"],
                           index=False,
                           mode='a',
    ...
    def Write_SPOT(self, CF, clear_all=False):

        if (len(self.BehaviorSPOTmsgs) >= self.msg_limit) or clear_all:
            df = pd.DataFrame(self.BehaviorSPOTmsgs)
            ...
            #reorder data elements
            df = df[self.Behavior_SPOT_col_order]
            ...
            df.to_csv(path_or_buf = CF.Control['SPOTBehaviorFile'],
                           index=False,
                           mode='a',
                           header = self.headerSPOT,
                            )

            df = pd.DataFrame(self.TravelSPOTmsgs)
            df['spd'] = df['spd'].map(lambda x: '%.2f' % x)

            #reorder data elements
            df = df[self.Travel_SPOT_col_order]
            ...
            df.to_csv(path_or_buf = CF.Control['SPOTTravelFile'],
                           index=False,
                           mode='a',
                           header = self.headerSPOT,
                            )

            self.headerSPOT = False

    def Write_CAM(self, CF, clear_all=False):

        if (len(self.CAMs) >= self.msg_limit) or clear_all:
            ...
            df_CAMs = pd.DataFrame(self.CAMs, columns=col)
            ...
            df_CAMs.to_csv(path_or_buf =CF.Control["CAMTransFile"], index=False,  mode='a', header = self.headerCAM)
