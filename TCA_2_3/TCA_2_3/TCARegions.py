...
class RegionsTest(unittest.TestCase):
    ...
    def test_event(self):
        ...
        assert self.rc.Event_regions[0].check('temp', tp=2, x=1, y=1) == 35
        assert self.rc.Event_regions[0].check('temp', 3, 1, 1) == -1234
        assert self.rc.Event_regions[0].check('temp', 20, 1, 1) == 29
        assert self.rc.Event_regions[0].check('temp', 250, 1, 1) == -9999

        assert self.rc.Event_regions[0].check('wipers', 2, 1, 1) == 0
        assert self.rc.Event_regions[0].check('wipers', 20, 1, 1) == 1
