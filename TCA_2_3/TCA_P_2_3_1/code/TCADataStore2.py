...
class DataStorage(object):
    ...
    def __init__(self, RandomGen, accel_included=True, link_included = False):
        """
    ...
    def pull_veh_data(self, veh, CF, RandomGen, Regions, tp):
        ...
        if (veh['vehicle_ID'] in self.db.keys()) and (self.db[veh['vehicle_ID']] is not None):

            self.timer.start('update_vehicle')

            self.timer.start('update_vehicle_Core_cols')
            ...
            self.timer.stop('update_vehicle_Core_cols')
            self.timer.start('update_vehicle_distance')
            ...
            self.timer.stop('update_vehicle_distance')
            self.timer.start('update_vehicle_acc')
            ...
            self.timer.stop('update_vehicle_acc')

            self.timer.start('update_vehicle_heading')
            ...
            self.timer.stop('update_vehicle_heading')

            veh_data['time'] = tp

            self.timer.stop('update_vehicle')

        else:
             #if new vehicle
            if (veh['vehicle_ID'] not in self.db.keys()):
                ...
                if PDM_equipped or BSM_equipped or CAM_equipped or SPOT_equipped:
                    ...
                    if 'link_name' in veh.keys():
                        link_name = veh['link_name']
                    else:
                        link_name = 'NA'
                    ...
                    if PDM_equipped:
                       self.db[veh['vehicle_ID']]['total_time_in_network'] = 0.0
                       self.db[veh['vehicle_ID']]['time_out_network'] = 0.0
                       self.db[veh['vehicle_ID']]['vehicle_ID_buffer_empty'] = True
                       self.db[veh['vehicle_ID']]['vehicle_SS_buffer_empty'] = True
                       self.db[veh['vehicle_ID']]['SS_count_generated_by_vehicle'] = 0
                       self.db[veh['vehicle_ID']][ 'SS_count_in_vehicle'] = 0
                       self.db[veh['vehicle_ID']]['time_to_next_periodic'] = 0.0
                       self.db[veh['vehicle_ID']]['distance_to_next_periodic'] = 0.0
                       self.db[veh['vehicle_ID']]['time_motionless'] = 0.0
                       self.db[veh['vehicle_ID']]['time_of_last_stop'] = -1000
                       self.db[veh['vehicle_ID']]['distance_of_last_stop'] = 1000.0
                       self.db[veh['vehicle_ID']]['time_stamp_of_ID'] = -1000
                       self.db[veh['vehicle_ID']]['PSN'] = RandomGen['psn']
                       self.db[veh['vehicle_ID']]['looking_for_start'] = False
                       self.db[veh['vehicle_ID']]['in_privacy_gap'] = False
                       self.db[veh['vehicle_ID']]['privacy_gap_start'] = 0
                       self.db[veh['vehicle_ID']]['distance_in_privacy_gap'] = 0.0
                       self.db[veh['vehicle_ID']]['PSN_time_to_end_gap'] = 0.0
                       self.db[veh['vehicle_ID']]['PSN_start_time'] = 0.0
                       self.db[veh['vehicle_ID']]['PSN_start_distance'] = 0.0
                       self.db[veh['vehicle_ID']]['PSN_distance_to_change'] = float(CF.Strategy["DistanceBetweenPSNSwitches"])
                       self.db[veh['vehicle_ID']]['PSN_change_ID'] = 0
                       self.db[veh['vehicle_ID']]['PSN_distance_to_end_of_gap'] = 0.0
                       self.db[veh['vehicle_ID']]['PSN_time_to_end_of_PSN'] = float(veh['time']) + float(CF.Strategy['TimeBetweenPSNSwitches'])
                       self.db[veh['vehicle_ID']]['new_distance'] = 0.0
                       self.db[veh['vehicle_ID']]['time_of_start_snapshot'] = 0
                       self.db[veh['vehicle_ID']]['time_of_periodic_snapshot'] = 0
                       self.db[veh['vehicle_ID']]['prev_time_PDM_dsrc_transmit'] = -1000
                       self.db[veh['vehicle_ID']]['prev_time_PDM_cellular_transmit'] = -1000
                       self.db[veh['vehicle_ID']]['dsrc_transmit_pdm'] = False
                       self.db[veh['vehicle_ID']]['last_RSE_transmitted_to'] = -1


                    if BSM_equipped:
                        self.db[veh['vehicle_ID']]['prev_time_BSM_dsrc_transmit'] = -1000
                        self.db[veh['vehicle_ID']]['prev_time_BSM_cellular_transmit'] = -1000
                        self.db[veh['vehicle_ID']]['BSM_tmp_ID'] = RandomGen['BSM_Tmp_ID']
                        self.db[veh['vehicle_ID']]['BSM_time_to_ID_chg'] = float(veh['time']) + 300.0
                        self.db[veh['vehicle_ID']]['dsrc_transmit_bsm'] = False
                        self.db[veh['vehicle_ID']]['brake_status'] = '0000'
                        self.db[veh['vehicle_ID']]['brake_pressure'] = 0.0
                        self.db[veh['vehicle_ID']]['hard_braking'] = 0
                        self.db[veh['vehicle_ID']]['sent_percent888'] = 0

                    if CAM_equipped:
                        self.db[veh['vehicle_ID']]['dsrc_transmit_cam'] = False
                        self.db[veh['vehicle_ID']]['last_CAM_distance'] = 0.0
                        self.db[veh['vehicle_ID']]['last_CAM_speed'] = None
                        self.db[veh['vehicle_ID']]['last_CAM_heading'] = None
                        self.db[veh['vehicle_ID']]['last_CAM_time'] = None
                        self.db[veh['vehicle_ID']]['SPOT_Station_ID'] = RandomGen['SPOT_Station_ID']


                    if SPOT_equipped:
                        self.db[veh['vehicle_ID']]['prev_SPOT_distance'] = None
                        self.db[veh['vehicle_ID']]['prev_SPOT_speed'] = None
                        self.db[veh['vehicle_ID']]['prev_SPOT_heading'] = None
                        self.db[veh['vehicle_ID']]['prev_SPOT_accel'] = None
                        self.db[veh['vehicle_ID']]['max_SPOT_accel'] = 0.0
                        self.db[veh['vehicle_ID']]['max_SPOT_yawrate'] = 0.0
                        self.db[veh['vehicle_ID']]['SPOT_accel_tp'] = None
                        self.db[veh['vehicle_ID']]['SPOT_accel_X'] = 0.0
                        self.db[veh['vehicle_ID']]['SPOT_accel_Y'] = 0.0
                        self.db[veh['vehicle_ID']]['SPOT_accel_v'] = 0.0
                        self.db[veh['vehicle_ID']]['SPOT_accel_heading'] = 0.0
                        self.db[veh['vehicle_ID']]['SPOT_accel_yawrate'] = 0.0
                        self.db[veh['vehicle_ID']]['SPOT_yawrate_tp'] = None
                        self.db[veh['vehicle_ID']]['SPOT_yawrate_X'] = 0.0
                        self.db[veh['vehicle_ID']]['SPOT_yawrate_Y'] = 0.0
                        self.db[veh['vehicle_ID']]['SPOT_yawrate_v'] = 0.0
                        self.db[veh['vehicle_ID']]['SPOT_yawrate_heading'] = 0.0
                        self.db[veh['vehicle_ID']]['SPOT_yawrate_accel'] = 0.0
                        self.db[veh['vehicle_ID']]['SPOT_trans_tp'] = None
                        self.db[veh['vehicle_ID']]['prev_tp_travel_SPOT'] = None
                        self.db[veh['vehicle_ID']]['prev_tp_accel_SPOT'] = None
                        self.db[veh['vehicle_ID']]['prev_tp_yawrate_SPOT'] = None


                    if link is not None:
                         self.db[veh['vehicle_ID']]['link'] = link

                    if link_name is not None:
                         self.db[veh['vehicle_ID']]['link_name'] = link_name
                    ...
                    self.timer.stop('new_vehicle')

                else:
                    self.db[veh['vehicle_ID']] = None
