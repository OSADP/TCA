        ...
        if len(data)>4:
            ...
            try:
                veh_dict = {
                          ...
                          'location_x': float(data[self.cols['x_col'] ].strip()) * self.unit_converter,
                          'location_y': float(data[self.cols['y_col']].strip()) * self.unit_converter,
                          }
                ...
                if self.cols['link_col'] is not None:
                    veh_dict['link'] = data[self.cols['link_col']].strip()
                return veh_dict
                # if self.cols['acc_col'] == None and self.cols['type_col'] == None:
                #     return {
                #           'vehicle_ID': data[self.cols['veh_col']].strip(),
                #           'time': float(data[self.cols['time_col']].strip()),
                #           'speed': float(data[self.cols['speed_col']].strip()),
                ...
                #           'location_y': float(data[self.cols['y_col']].strip()) * self.unit_converter,
                #     }
                # elif self.cols['acc_col'] == None:
                #     return {
                #           'vehicle_ID': data[self.cols['veh_col']].strip(),
                #           'time': float(data[self.cols['time_col']].strip()),
                #           'type': float(data[self.cols['type_col']].strip()),
                #           'speed': float(data[self.cols['speed_col']].strip()),
                #           'location_x': float(data[self.cols['x_col'] ].strip()) * self.unit_converter,
                #           'location_y': float(data[self.cols['y_col']].strip()) * self.unit_converter,
                #     }
                # elif self.cols['type_col'] == None:
                #     return {
                #           'vehicle_ID': data[self.cols['veh_col']].strip(),
                #           'time': float(data[self.cols['time_col']].strip()),
                #           'accel_instantaneous': float(data[self.cols['acc_col']].strip()),
                #           'speed': float(data[self.cols['speed_col']].strip()),
                #           'location_x': float(data[self.cols['x_col'] ].strip()) * self.unit_converter,
                #           'location_y': float(data[self.cols['y_col']].strip()) * self.unit_converter,
                #     }
                ...
                #           'time': float(data[self.cols['time_col']].strip()),
                #           'accel_instantaneous': float(data[self.cols['acc_col']].strip()),
                #           'type': data[self.cols['type_col']].strip(),
                #           'speed': float(data[self.cols['speed_col']].strip()),
                #           'location_x': float(data[self.cols['x_col'] ].strip()) * self.unit_converter,
                #           'location_y': float(data[self.cols['y_col']].strip()) * self.unit_converter,
                #     }
