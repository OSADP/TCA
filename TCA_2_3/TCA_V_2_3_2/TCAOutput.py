...
#----------------------------------------------------------------------
    def WriteSPOTList(self, TravelSPOTList, BehaviorSPOTList, CF):
        ...
        with open(self.TravelSPOToutfile, 'w') as f_out:
            f_out.write("Time_Taken,Vehicle_ID,X,Y,Speed\n")
            for SPOT in TravelSPOTList:
                f_out.write('%d,%s,%.5f,%.5f,%.2f\n' % (SPOT['localtime'], SPOT['vehicle_ID'], SPOT['x'], SPOT['y'], SPOT['spd']))
        ...
        with open(self.BehaviorSPOToutfile, 'w') as f_out:
            f_out.write("Time_Taken,Vehicle_ID,X,Y,Speed,Acceleration,Heading,YawRate\n")
            for SPOT in BehaviorSPOTList:
                f_out.write('%d,%s,%.5f,%.5f,%.2f,%.2f,%s,%.1f\n' % (SPOT['localtime'], SPOT['vehicle_ID'], \
