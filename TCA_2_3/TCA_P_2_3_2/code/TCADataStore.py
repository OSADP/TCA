...
class DataStorage(object):
        ...
        if (CF.Control['PDMVehicleTypes'] != []) or (CF.Control['BSMVehicleTypes'] != []) \
            ...
            or (CF.Control['SPOTVehicleTypes'] != []):
                ...
                if 'type' in veh.keys():
                    ...
                    print "ERROR: Cannot assign vehicle equipage by vehicle type when type is not an element of vehicle trajectory data"
                    sys.exit()
    ...
    def pull_veh_data(self, veh, CF, RandomGen, Regions, tp, msgs):
        ...
        if (veh['vehicle_ID'] in self.current_veh.keys()) and (self.current_veh[veh['vehicle_ID']] is not None):
            ...
            # Only change heading if the vehicle has moved since the last timestep
            ...
            if (veh['vehicle_ID'] not in self.current_veh.keys()):
                ...
                if PDM_equipped or BSM_equipped or CAM_equipped or SPOT_equipped:
                    ...
                    if CAM_equipped:
                        ...
                        self.current_veh[veh['vehicle_ID']]['last_CAM_time'] = None
