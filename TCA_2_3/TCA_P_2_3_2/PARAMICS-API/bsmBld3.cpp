...
//	if(wholeSecond == 1)
	...
	//if(fmod(simTime,1) == 0)
	{
		...
		if(cumuVehCount <= numVehicles)
		{
			...
			g_vI[i].linkID = qpg_LNK_index(qpg_VHC_link(vehicle));
				...
				try
				{
					for(i = 0; i < cumuVehCount; i++)
					{
						...
						if(tempL != NULL && g_vI[i].vehicleID > 0)
						{
							...
							oneVehList.append(g_vI[i].linkID);
				...
				if(vehList)
				{
					...
					for(i = 0; i < vehCount; i++)
					{
						...
						for(j = 0; j < 9; j++){
							...
							if(j == 8 )// || j == 2 || j == 4 || j == 5)
							{ 
								int dummyTy = extract<double>(oneVehList[j]);
								qps_GUI_printf(" %d,",dummyTy);
							}
						}
						qps_GUI_printf("\n");
					}*/
								...
								for(j = 0; j < numItemsList; j++)
								{
									switch(j)
									{
										...
										case 1:
											...
											//vehColorLinkID = extract<int> (vehColorList[numItemsList*i+j]);
											//**qps_GUI_printf("link flag %d %f\n",vehColorLinkID, tp);
											//qps_GUI_printf("link flag %s %f\n",(char*)vehColorLinkChar.c_str(), tp);
											break;
										case 2:
											vehColor = extract<int> (vehColorList[numItemsList*i+j]);
											break;
										default:
											vType = extract<int> (vehColorList[numItemsList*i+j]);
											//qps_GUI_printf("type flag %d %f\n",vType, tp);
									}
								}
								VEHICLE* vehColored = qpg_VHC_lookupV(qpg_NET_link((char*)vehColorLinkChar.c_str())
									, vehColorID);
//								VEHICLE* vehColored = qpg_VHC_lookupV(qpg_NET_linkByIndex(vehColorLinkID), vehColorID);
								if(vehColored != NULL){ 
									switch(vehColor)
									{
										case 0: // BSMTransColor / BSMDefaultColor
											qps_DRW_vehicleColour(vehColored,API_DARKBLUE);
											break;
										case 1: // PDMStartColor
											qps_DRW_vehicleColour(vehColored,API_GREEN);
											break;
										case 2: // PDMStopColor
											qps_DRW_vehicleColour(vehColored,API_RED);
											break;
										case 3: // PDMDSRCTransColor
											qps_DRW_vehicleColour(vehColored,API_GREY1);
											break;
										case 4: // PDMPeriodicColor
											qps_DRW_vehicleColour(vehColored,API_MEDIUMPURPLE);//qps_DRW_vehicleColour(vehColored,API_LAVENDER);
											break;
										case 5: // Default
											qps_DRW_vehicleColour(vehColored,API_WHITE);
											break;
										case 6: // PDMCellularTransColor
											qps_DRW_vehicleColour(vehColored,API_MEDIUMBLUE);//qps_DRW_vehicleColour(vehColored,API_LIGHTBLUE);
											break;
										case 7: // DualPDMBSMColor
											qps_DRW_vehicleColour(vehColored,API_WHITE);
											break;
										case 8: // PDM only
											qps_DRW_vehicleColour(vehColored,API_WHITE);
											break;
										case 9: // CAMTransColor
											qps_DRW_vehicleColour(vehColored,API_ORANGE);
											break;
										case 10: // SpotBehaviorColor
											qps_DRW_vehicleColour(vehColored,API_VIOLET);
											break;
										case 11: // SpotTravelColor
											qps_DRW_vehicleColour(vehColored,API_BLUE);
											break;
										case 12: // SpotTransColor
