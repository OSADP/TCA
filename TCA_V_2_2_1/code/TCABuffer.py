...
class CLBSMAllBuffer:
    ...
    def next(self):
        return self.df[ self.df['used'] == 0].index[0]
...
#Creates the Buffer class. Requires a capacity to start
    ...
    #9.2 Manage Buffer Contents
        ...
        elif (self.BufferCount(vehicleID) <= (self.TotalCapacity - 1)):
    ...
    def PrintSS(self, SS):
        print "%s,%d,%d,%s,%s,%s,%s,%s,%s,%d" % (
            ...
            SS["type"], SS["transtime"], SS["transTo"], SS["deltime"], SS["transmission_received_time"])
    ...
    #prints all buffer SS
    def TransmitPDM(self, vehicleID, isCellular, transTo, tp, Snapshots, CF, latency):
    #10.0
        if isCellular:
        ...
        for PSN in PSN_list:
            ...
            for SS in sort_by_PSN[PSN]:
                ...
                #latency is 0 if transmitted via RSE
...
class BufferContentCheck(unittest.TestCase):
    ...
    def test_add(self):
        ...
        assert self.buffer.BufferCount(self.SS["VID"]) == 1

    def test_remove(self):
        self.buffer.AddSS(self.SS["VID"], self.SS, self.SS["time"], self.Snapshots, self.CF, False)
        assert self.buffer.BufferCount(self.SS["VID"]) == 1
        self.buffer.RemoveSS(self.SS["VID"], 1, 111, self.Snapshots, self.CF)
        assert self.buffer.BufferCount(self.SS["VID"]) == 0

    def test_bufferCount(self):
        self.buffer.AddSS(self.SS["VID"], self.SS, self.SS["time"], self.Snapshots, self.CF, False)
        self.buffer.AddSS(self.SS["VID"], self.SS, self.SS["time"], self.Snapshots, self.CF, False)
        self.buffer.AddSS(self.SS["VID"], self.SS, self.SS["time"], self.Snapshots, self.CF, False)
        assert self.buffer.BufferCount(self.SS["VID"]) == 3

    def test_transmit(self):
        self.buffer.AddSS(self.SS["VID"], self.SS, self.SS["time"], self.Snapshots, self.CF, False)
        self.buffer.TransmitPDM(self.SS["VID"], False, 0, self.SS["time"], self.Snapshots, self.CF, 0)
