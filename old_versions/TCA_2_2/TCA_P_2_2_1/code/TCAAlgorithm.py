...
class TCA_Algorithm:
    ...
    #-------------------------------------------------------------------------
    def GenerateTransmitBSM(self, row, transTo, BSMs, Outlevel, AccelColumn, regions, RandomGen=None, latency=0, checkCellular=False):
        if row['BSM_equipped']:
            transmitted = True
    ...
    def CheckCellular(self, df, CF, RandomGen, checkPDM, pdm, tp, regions):
        ...
        if pdm and checkPDM:
            ...
            for vehicle_id in sorted(df_pdm_cellular.index):
                ...
                self.timer.stop('cell_PDM')


    #-------------------------------------------------------------------------
    def CheckDSRC(self, df, CF, checkPDM, pdm, tp, regions):
        ...
        if len(df_active) > 0:
            ...
            for RSE in RSE_Ranges.keys():
                ...
                rse_active.apply(lambda x: self.GenerateTransmitBSM(x, RSE, self.BSMs, CF.Control["OutputLevel"],\
                                                                CF.Control["AccelColumn"], regions,), axis=1)
                    ...
                    for vehicle_id in RSE_Ranges[RSE]:
                        ...
                        if entry['PDM_equipped'].any():
                            if self.PDMBuffer.BufferCount(vehicle_id) >= CF.Strategy["MinNumberofPDMtoTransmitViaDSRC"]:
                                ...
                                df['time_of_last_transmit'][vehicle_id] = entry["time"]
    ...
    def RSETransmitPDM(self, df, vehicle_id, CF, RSE, entry, Buffer, Snapshots):
        ...
        if (entry["time"] != 0):
            ...
            Buffer.TransmitPDM(vehicle_id, False, RSE, entry["time"].values[0], Snapshots, CF, 0 )
