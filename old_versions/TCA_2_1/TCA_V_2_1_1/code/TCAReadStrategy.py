...
class LoadStrategyTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.CV.Control['strategy.xml'] = 'tca_delete_me_test_file_good.xml'
    ...
    def test_load(self):
        self.CV.Control['strategy'] = 'tca_delete_me_test_file_good.xml'
        errors = ReadStrategyFile(self.CV)
        assert len(errors) ==0
        assert self.CV.Strategy["RSEFlag"] == 0
        assert self.CV.Strategy["StartThreshold"] == 10
        assert self.CV.Strategy["DistanceBetweenPSNSwitches"] == 3281

    def test_load_bad(self):
        self.CV.Control['strategy'] = 'tca_delete_me_test_file_bad.xml'
        errors = ReadStrategyFile(self.CV)
        assert len(errors) == 3
