...
control_values =  {
            ...
            'Link_Included' : [False, 'Default', '', 'None', 'InputFiles/TrajectoryFile/Link_Included'],
            ...
            #DIDC
            'DIDCParametersFile' : [None, 'Default', '', 'file', 'InputFiles/DIDCParametersFile'],
            'BMMTransFile' : ['BMM_Trans.csv', 'Default', '', 'file', 'OutputFiles/BMMTransFile'],
            'DIDCVehicleIDs' : [[], 'Default', '', 'List_int', 'EquippedVehicles/DIDCVehicleIDs'],
            'DIDCVehicleTypes' : [[], 'Default', '', 'List_int', 'EquippedVehicles/DIDCVehicleTypes'],
            'DIDCMarketPenetration' : [0, 'Default', '', 'int', 'EquippedVehicles/DIDCMarketPenetration'],
            'PeriodicGMTFile' : ['periodic_gmt_output.csv', 'Default', '','file','OutputFiles/PeriodicGMTFile'],
        }
...
def write_default_didc(filename):
    with open(filename, 'wb') as f_out:
        f_out.write("""<?xml version="1.0" encoding="UTF-8" ?>
<DIDC>
    <Title>DIDC Parameter File</Title>
    <!-- Transmission Threshold is how many BMMs the BMM Buffer must contain before it transmits Default = 4 -->
    <TransmissionThreshold>4</TransmissionThreshold>
    <PeriodicGenerationMeanTime>300</PeriodicGenerationMeanTime>
    <!-- Custom User Triggers:
    The Title, Start, End, and Type tags are required. GenerationMeanTime, SDAfterEventEnd, and MedianPostTriggerReports are optional and if not included, will be set to their default values.
    For less than and greater than in the start/end trigger statements, use &lt; (<) and &gt; (>). -->
    <Triggers>
        <Trigger>
            <Title>Trigger 1</Title>
            <!-- Use the start tag for your start trigger statement. 
            Variables that can always be used: yawrate, speed, heading, acceleration, and time. Regional attributes can also be used when they are defined in the regions file.-->
            <Start>yawrate &gt;= 9.0</Start>
            <!-- Use the end tag for your end trigger statement. -->
            <End>yawrate &lt; 9.0</End>
            <!-- Type is the number that identifies BMMs by the trigger that generated it.--> 
            <Type>1</Type>
            <!-- GenerationMeanTime is the mean generation time(seconds) in between snapshots. Default is 50 -->
            <GenerationMeanTime>50</GenerationMeanTime>
            <!-- SDAfterEventEnd is the standard deviation ????? Default is 0.25. --> 
            <SDAfterEventEnd>0.25</SDAfterEventEnd>
            <!-- MedianPostTriggerReports is the median number of messages generated after the end of an event. Default is 4. -->
            <MedianPostTriggerReports>4</MedianPostTriggerReports>
            <!-- TargetSamplingRate is the target messages wanted per time period. The default is None. -->
            <TargetSamplingRate>None</TargetSamplingRate>
        </Trigger>
        <Trigger>
        ...
        <Trigger>
            ...
            <Start>speed &lt; 15 and yawrate &gt; 5.0 </Start>
            <End> speed &gt; 15 or yawrate &lt; 5.0 </End>
            <Type>3</Type>
        </Trigger>
    </Triggers>
</DIDC>""")
