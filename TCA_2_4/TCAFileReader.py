...
class Trajectories(object):
    ...
    def read_header(self, line, CF):
        ...
        try:
            self.cols['length'] = header.index(length)
            self.cols['width'] = header.index(width)
        except:
            self.cols['length'] = None
            self.cols['width'] = None
        ...
        if len(data)>4:
            ...
            try:
                ...
                if self.cols['length'] is not None:
                    veh_dict['length'] = float(data[self.cols['length']].strip())
                if self.cols['width'] is not None:
                    veh_dict['width'] = float(data[self.cols['width']].strip())
